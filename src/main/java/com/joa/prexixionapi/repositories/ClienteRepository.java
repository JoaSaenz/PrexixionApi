...
public class ClienteRepository {
    ...
    private void appendFilters(StringBuilder query, ClienteDataTablesRequest req) {
        if (req.getSearch() != null && !req.getSearch().trim().isEmpty()) {
            query.append(
                    " AND (cl.ruc LIKE :search OR cl.razonSocial LIKE :search OR cl.nombreCorto LIKE :search OR ge.descripcion LIKE :search  OR cl.codigoCliente LIKE :search) ");
        }
        ...
        if (cge != null && !cge.trim().isEmpty()) {
            query.append(
                    " AND (CASE WHEN cl.idGrupoEconomico IS NOT NULL AND cl.idGrupoEconomico NOT IN (0, 1) THEN 1 ELSE 0 END) IN (:categoriaGrupoE) ");
        }
    ...
    private List<Cliente> mapTuples(List<Tuple> tuples) {
        ...
        for (Tuple tuple : tuples) {
            ...
            obj.setContribuyente(new Gclass(
                    tuple.get("idContribuyente", Integer.class) == null ? 0
            ...
            obj.setGrupoEconomico(new Gclass(
                    tuple.get("idGrupoEconomico", Integer.class) == null ? 0
                            : tuple.get("idGrupoEconomico", Integer.class),
