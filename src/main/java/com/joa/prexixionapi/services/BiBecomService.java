...
public class BiBecomService {
    ...
    public BiBecomDashboardDTO getDashboardData(String periodo) {
        ...
        // 1. Extraer año del periodo (ej: de "2026-07" o "202607" se obtiene "2026")
        String anio = String.valueOf(LocalDate.now().getYear());
        if (periodo != null && periodo.trim().length() >= 4) {
            anio = periodo.trim().substring(0, 4);
        }
        ...
        BiBecomHeaderDTO headerDTO = BiBecomHeaderDTO.builder()
                ...
                .periodoNombre(periodo != null && !periodo.trim().isEmpty() ? periodo : "Julio 2026")
        ...
        List<BiBecomDevolucionItemDTO> denegadasList = allDevoluciones.stream()
                .filter(item -> item.getIdEstado() != null && (item.getIdEstado() == 2 || item.getIdEstado() == 5)
                             && item.getIdResultado() != null && (item.getIdResultado() == 4 || item.getIdResultado() == 5))
        ...
        // KPI 2: Clientes Activos (idEstadoCliente=1) & Actualización Terminada (idEstado=4)
        ...
        // KPI 3: Clientes Activos (idEstadoCliente=1) & Actualización En Proceso (idEstado=3)
        int totalEnProcesoAct = (int) allActualizaciones.stream()
                .filter(item -> item.getIdEstadoCliente() != null && item.getIdEstadoCliente() == 1
                             && item.getIdEstado() != null && item.getIdEstado() == 3)
                .count();

        // KPI 4: Clientes Activos (idEstadoCliente=1) & Actualización Pendiente (idEstado=2)
        int totalPendientesAct = (int) allActualizaciones.stream()
                .filter(item -> item.getIdEstadoCliente() != null && item.getIdEstadoCliente() == 1
                             && item.getIdEstado() != null && item.getIdEstado() == 2)
                .count();

        // KPI 5: Clientes Retirados (idEstadoCliente=4) & Actualización Terminada (idEstado=4)
        int totalRetiradosSinPagoAct = (int) allActualizaciones.stream()
                .filter(item -> item.getIdEstadoCliente() != null && item.getIdEstadoCliente() == 4
                             && item.getIdEstado() != null && item.getIdEstado() == 4)
                .count();

        // Tabla 1: Clientes Activos (idEstadoCliente=1) con estados Terminado(4), En Proceso(3) o Pendiente(2)
        List<BiBecomActualizacionItemDTO> activosList = allActualizaciones.stream()
                .filter(item -> item.getIdEstadoCliente() != null && item.getIdEstadoCliente() == 1
                             && item.getIdEstado() != null && (item.getIdEstado() == 4 || item.getIdEstado() == 3 || item.getIdEstado() == 2))
                .collect(Collectors.toList());
        ...
        BiBecomActualizacionesDTO actualizacionesDTO = BiBecomActualizacionesDTO.builder()
                ...
                .totalRetiradosSinPago(totalRetiradosSinPagoAct)
