...
public class AttendanceService {
    ...
    public List<AttendanceDTO> getDailyAttendance(String fecha, List<Integer> statuses, List<Integer> types) {
        ...
        if (statuses != null && !statuses.isEmpty()) {
            queryPersonal.append(" AND p.idEstado IN (")
                    .append(statuses.stream().map(String::valueOf).collect(Collectors.joining(","))).append(")");
        } else {
            queryPersonal.append(" AND p.idEstado IN (1, 2)"); // Por defecto activos/vigentes
        }

        if (types != null && !types.isEmpty()) {
            queryPersonal.append(" AND p.idTipo IN (")
                    .append(types.stream().map(String::valueOf).collect(Collectors.joining(","))).append(")");
        }

        List<AttendanceDTO> personalActivo = jdbcTemplate.query(queryPersonal.toString(),
                (rs, rowNum) -> AttendanceDTO.builder()
                        .dni(rs.getString("dni"))
        ...
        log.info("Matching markings for {} personnel. Total markings found: {}", personalActivo.size(),
                marcaciones.size());
    ...
    private int calcularMinutosTrabajados(String mi, String ms, String ti, String ts) {
        try {
            ...
            // Regla heurística: Si el turno es >= 6 horas (360 min), descontar 1 hora de
            // almuerzo
            if (total >= 360) {
                total -= 60;
            }
    ...
    public AttendanceStatsDTO getStats(String fechaI, String fechaF) {
        ...
        List<AttendanceStatsDTO.AreaLatenessDTO> areasTardonas = groupedByArea.entrySet().stream()
                .map(entry -> AttendanceStatsDTO.AreaLatenessDTO.builder()
                        ...
                        .totalMinutos(entry.getValue().stream()
                                .mapToInt(AttendanceStatsDTO.PersonLatenessDTO::getMinutosTardanza).sum())
    ...
    /**
     * Obtiene el historial completo de marcaciones de un trabajador en una fecha
     * específica.
     */
